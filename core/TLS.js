...
export async function decrypt_tls_responseV6(encRecords, key, IV){
  ...
  for (let i=0; i < encRecords.length; i++){
    ...
    let witnesses = JSON.stringify({
      "ciphertext": uint8ArrayToStringArray(rec.slice(8)),
      "key": uint8ArrayToStringArray(key),
      "nonce": uint8ArrayToStringArray(nonce)
    });
    fs.writeFileSync('ephemeral_data/input_' + i + '.json', witnesses);
    let nonceHex = Buffer.from(nonce).toString('hex');
    fs.writeFileSync('ephemeral_data/tag_iv_' + i + '.txt', nonceHex);
    let aadHex = Buffer.from(aad).toString('hex');
    fs.writeFileSync('ephemeral_data/aad_' + i + '.txt', aadHex);
