...
const MAX_FRAGMENT_LENGTH_VALUE = Object.freeze({
  512: 0x01,
  1024: 0x02,
  2048: 0x03,
  4096: 0x04,
...
export class TLS {
  constructor (serverName, port, headers, options){
    ...
    this.maxFragmentLength = options.maxFragmentLength;
  ...
  buildClientHello(){
    ...
    tmp = [];
    if (this.maxFragmentLength !== undefined) {
      ...
      tmp.push(MAX_FRAGMENT_LENGTH_VALUE[this.maxFragmentLength]);
    }
  ...
  async receiveAndParseServerHello(){
    ...
    let c = [];
    // check if certificate spans more than 1 record
    if (reclenMhm === this.maxFragmentLength && clen > this.maxFragmentLength) {
      // parse the first part of the certificate until the end of the record
      const cPart1 = s.slice(p, p+=this.maxFragmentLength);
      // then check the header of the next record
      assert(eq(s.slice(p, p+=3), [0x16, 0x03, 0x03]));
      // parse the length of the message
      const secRecLen = ba2int(s.slice(p, p+=2));
      // the rest of the message will be the second part of the certificate
      const cPart2 = s.slice(p, p+=secRecLen);
      // concatenate two parts of the certificate and parse it
      c = concatTA(cPart1, cPart2);
      await this.parseCertificate(c);
    } else {
      if (!this.isMhm) {
        assert(reclenMhm == clen+4);
      }

      c = s.slice(p, p + 4 + clen);
      const cParsedBytes = await this.parseCertificate(c);
      p += cParsedBytes;
    }
...
function uint8ArrayToStringArray(input) {
  let result = [];
...
function uint8ArrayToSixByteStringArray(input) {
  let chunksize = 6;
  let result = [];
  for (let i = 0; i < input.length; i = i + chunksize) {
    let combined = 0;
    for (let j = 0; j < chunksize; j++) {
      if (j + i >= input.length ) {
        break
      }
      combined = combined*Math.pow(2,8) + input[j + i];
    }
    result.push(combined.toString())
  }
  assert(result.length === Math.ceil(input.length/chunksize));
  return result;
}
...
export async function decrypt_tls_responseV6(encRecords, key, IV, clientSwk, clientSwkHash){
  ...
  for (let i=0; i < encRecords.length; i++){
    ...
    plaintext.push( new Uint8Array (await crypto.subtle.decrypt(
      ...
      cryptoKey,
      rec.slice(8).buffer)));
    // console.dir(rec.slice(8), {'maxArrayLength': null});
    // console.dir(plaintext[plaintext.length - 1], {'maxArrayLength': null});
    ...
    inputsList.push(witnesses);
    const nonceHex = Buffer.from(nonce).toString('hex');
    tagIvList.push(nonceHex);
    const aadHex = Buffer.from(aad).toString('hex');
    aadList.push(aadHex);

  }
  return { plaintext, inputsList, tagIvList, aadList };
}
