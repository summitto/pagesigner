...
import {
  sortKeys, assert, bytesToBits, concatTA, int2ba, xor, ba2int,
  splitIntoChunks, eq,
} from '../utils.js';
...
export class GHASH {
  constructor(noOfAESBlocks, ot){
    /** @type {OTManager} */ this.ot = ot;
  ...
  processOTResponse(){
    assert(this.lastChoiceBits != null);
    const hisXTables = this.ot.requestData(...arrayToChoiceBits(this.lastChoiceBits));
  ...
  processStep3Response(resp){
    ...
    const res = this.blockAggregationProcessResponse();
  ...
  processFinResponse(ghashInputs){
    assert(this.lastChoiceBits != null);
    const twoXTables = this.ot.requestData(...arrayToChoiceBits(this.lastChoiceBits));
  ...
  blockAggregationBuildRequest(ghashInputs){
    assert(this.lastChoiceBits == null);
  ...
  blockAggregationProcessResponse(){
    assert(this.lastChoiceBits != null);
    const XTableEntries = this.ot.requestData(...arrayToChoiceBits(this.lastChoiceBits));
