...
import { OTManager, arrayToChoiceBits } from 'ot-wrapper';
...
import {ba2hex, assert, getRandom, bytesToBits, concatTA, int2ba, str2ba,
  ...
  pubkeyPEM2raw, checkExpiration } from '../utils.js';
...
export class TWOPC {
  constructor(notary, plaintextLen, circuits, progressMonitor) {
    // notary is an object {'IP':<notary IP address>, 'pubkeyPEM':<notary pubkey in PEM format>, 'otPort': <OT server port> }
    this.notary = notary;
    this.ot = new OTManager(false, false);
    ...
    // we need GHASH to first tell OT Manager how many OTs will be needed,
    // then we pass the OT Manager to GHASH
    this.ghash = new GHASH(noOfAESBlocks, this.ot);
  ...
  async run(cr, sr, allHandshakes, pmsShare) {
    ...
    const c4Out = (await this.runCircuit(input4, 4, [this.cwkShare, this.civShare]))[0];
  ...
  async checkServerFinished(encSFWithNonceAndTag, allHandshakes) {
    ...
    const ghashSF = new GHASH(1, this.ot);
    ghashSF.buildFinRequest(H1share);

    const step3resp = await this.send('c5_step3', concatTA(
      this.getDecommitment(5),
      encSF,
    ));
    assert(step3resp.length === 16);
    o = 0;
    ...
    const tagShare = ghashSF.processFinResponse([aad, encSF, lenAlenC]);
  ...
  async getTagFromPowersOfH(erb){
    ...
    await this.send('ghash_step1', concatTA(
      // send decommitment to the previous circuit
      this.getDecommitment(7),
      this.ghash.buildStep1()),
    );
    if (this.ghash.isStep1ResponseExpected()){
      this.ghash.processOTResponse();
    }
    if (this.ghash.isStep2Needed()){
      this.ghash.buildStep2();
      await this.send('ghash_step2');
      this.ghash.processOTResponse();
    }
    const req3 = concatTA(...ghashInputs, this.ghash.buildStep3(ghashInputs));
  ...
  async send(cmd, data = new Uint8Array(0), returnPromise = false, willEncrypt = true,
    ...
    const fetchPromise = fetch(
      'http://'+this.notary.IP+':'+this.notary.port+'/'+cmd+'?'+this.uid,
      {
        method: 'POST',
        mode: 'cors',
        cache: 'no-store',
    ...
    const promise = new Promise(async function(resolve, reject) {
      ...
      if (downloadProgress) {
        ...
        if (resp.status !== 200) {
          ...
          reject(new Error(`cmd ${cmd} failed: ${resp.status} ${resp.statusText}` + (error !== '' ? `, reason - ${error}` : '')));
        }
  ...
  async init() {
    ...
    const init1Blob = await this.send('init', concatTA(pubBytes, int2ba(this.C6Count, 2)), false, false, false);

    if (!init1Blob) {
      throw new Error('Notarization failed - failed to initialize 2-party computation!');
    }
  ...
  async phase5(c4out, masks4, verify_data) {
    ...
    // const otReq = this.ghash.buildFinRequest(H1share);
    this.ghash.buildFinRequest(H1share);
    ...
    const lenAlenC = new Uint8Array([0, 0, 0, 0, 0, 0, 0, 104, 0, 0, 0, 0, 0, 0, 0, 128]);
    const tagShare = this.ghash.processFinResponse([aad, encCF, lenAlenC]);
    // notary's gctr share is already included in notaryTagShare
  ...
  async runCircuit(inputs, cNo, extra) {
    console.log('in runCircuit', cNo);

    if (!this.ot.isConnected()) {
      this.ot.connect(`${this.notary.IP}:${this.notary.otPort}`);
    ...
    // Special case for C4 where the client includes cwk and civ choices for C6 in their C4 OT request
    // for all executions of C6. This means that "extra input" is duplicated C6 count times.
    // Upon receiving the OT response, the client separates C4 response from the partial C6 response with
    // cwk and civ. C6 parts are stored as state until the client receives the rest of the labels for C6.
    // Then those cwk and civ labels are prepended to the labels of every C6 execution.
    if (extra) {
      for (let i = 0; i < extra.length; i++){
        let bits = bytesToBits(extra[i]);
        if (typeof(extra[i+1]) == 'number'){
          // sometimes we need an amount of bits which is not a multiple of 8
          // in such cases the input we be followed by a number of bits to slice
          bits = bits.slice(0, extra[i+1]);
          i += 1;
        }
        customInputBits.push(...bits);
      }

      if (cNo === 4) {
        customInputBits = new Array(this.C6Count).fill(customInputBits).flat();
      }
    }
    const extraBlocks = customInputBits.length;
    ...
    const blob1 = await this.send(`c${cNo}_step1`, decommit);

    let allClientLabels = this.ot.requestData(...arrayToChoiceBits([...inputBitsE, ...customInputBits]));
    ...
    this.ot.respondWithData(Uint8Array.from(notaryLabels));
    ...
    if (cNo == 6){
      ...
      // "pre-step2".
      await this.send('c6_pre2', step2Payload, true);
      // zero out the data so that it doesn't get sent in step2
      step2Payload = new Uint8Array(0);
    }

    if (cNo == 4){
      // cuts off c6 partial response, stores it, returns c4 response
      allClientLabels = this.processC4Labels(allClientLabels, extraBlocks);
    }
    if (cNo == 6){
  ...
  processC4Labels(otResp, extraLabels) {
    const c4LabelsEnd = otResp.length - (extraLabels * 16);
    const c4Resp = otResp.slice(0, c4LabelsEnd);

    const c6PartResponse = otResp.slice(c4LabelsEnd);

    this.c6PartResponse = splitIntoChunks(c6PartResponse, c6PartResponse.length / this.C6Count);

    return c4Resp;
  }

  processC6Labels(otResp) {
    ...
    const labelsPerExecution = splitIntoChunks(otResp, otResp.length / this.C6Count);
    const updatedLabels = [];
    for (let i = 0; i < this.C6Count; i++) {
      updatedLabels.push(concatTA(
        this.c6PartResponse[i],
