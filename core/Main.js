...
export class Main{
  ...
  async queryNotaryNoSandbox(IP, port){
    // just fetch the pubkey and trust it
    const resp = await fetch('http://'+IP+':' + port + '/getPubKey', {
      method: 'POST',
      mode: 'cors',
      cache: 'no-store',
    });
  ...
  /**
   * @returns {Promise<{ host: string, request: string, response: string, timestamp: number, plaintext: Uint8Array[], ciphertext: Uint8Array[], inputsList: string[], tagIvList: string[], aadList: string[] }>}
   */
  async verifyPgsgV6(obj, useNotaryNoSandbox = true) {
